...
static ncclResult_t computeBuffSizes(struct ncclComm* comm) {
  ...
  if (comm->p2pChunkSize  > comm->buffSizes[NCCL_PROTO_SIMPLE]) comm->p2pChunkSize = comm->buffSizes[NCCL_PROTO_SIMPLE];
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent){
  ...
  NCCLCHECKGOTO(ncclTopoTuneModel(comm, graphs), ret, fail);
